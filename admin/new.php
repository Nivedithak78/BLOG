    ...
    <a class="navbar-brand" href="/index.php"><i class="fab fa-blogger"></i> BLOG</a>
      ...
      <div class="collapse navbar-collapse" id="navbarNav">
       ...
       <li class="nav-item">
        ...
        <li class="nav-item">
          <a class="nav-link active" href="new.php">Add</a>
        </li>
       </ul>
      </div>
  </div>
   ...
   <div class="container">
      <form method="POST" enctype="multipart/form-data">
          ...
          <div class="form-group">
              ...
              <input type="file" class="form-control forms" name="userimage">
          </div>
          <div class="form-group">
              <label class="heading">Video</label>
              <input type="file" class="form-control forms" name="uservideo">
          </div>
...
<?php
  if(isset($_POST["submit"])){
     ...
     $uservideo = $_FILES["uservideo"]["name"];
    
     if($title == "" && $content == ""){
       echo "you didn't enter anything";
     }else{
    $sql = "INSERT INTO posts(title, content, image, video) VALUES ('".$title."', '".$content."', '".$userimage."', '".$uservideo."')";
    if(!mysqli_query($conn, $sql)){
        echo "something went wrong";
    }else{
        move_uploaded_file($_FILES["userimage"]["tmp_name"], "../user/upload/".$_FILES["userimage"]["name"]);
        move_uploaded_file($_FILES["videos"]["tmp_name"], "../user/upload/".$_FILES['uservideo']['name']);
  ...
  if(isset($_GET["success"])){
    echo "<h6 class = 'new'>Recorded successfully</h6>";
  }
  ...
  if(isset($_POST["submit"])){
    ...
    if($title == "" && $content == ""){
      ...
      $sql = "INSERT INTO admin(title, content, image, video) VALUES ('".$title."', '".$content."', '".$userimage."', '".$uservideo."')";
      if(!mysqli_query($conn, $sql)){
        echo "something went wrong";
      }else{
        move_uploaded_file($_FILES["userimage"]["tmp_name"], "../user/upload/".$_FILES["userimage"]["name"]);
